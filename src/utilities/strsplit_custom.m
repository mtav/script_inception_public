...
function [cstr, matches] = strsplit_custom(str, del, varargin)
  ...
  try
    % try original function if available
    if exist('del', 'var')
      [cstr, matches] = strsplit(str, del, varargin{:});
    else
      [cstr, matches] = strsplit(str);
    end
    ...
    % else, try code ported from GNU Octave
    ...
    for n = 1:2:numel (params)
      ...
      elseif (ischar (varargin{n}))
        error ('strsplit:invalid_parameter_name', 'strsplit: invalid parameter name, %s', varargin{n});
      else
        print_usage ();
      end %endif
    end %endfor
    ...
    if (is_simple)
      % TODO: implement do_string_escapes somehow (does not exist in Matlab)
      %if (iscellstr (del))

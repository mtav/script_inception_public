% Source file taken from GNU Octave 4.0.0 and "#" comments replaced with Matlab-compatible "%".
%
...
%% xp = [0:10];
%% yp = sin (2*pi*xp/5);
%% lin = interp1Custom (xp, yp, xf);
%% near = interp1Custom (xp, yp, xf, "nearest");
%% previous = interp1Custom (xp, yp, xf, "previous");
%% next = interp1Custom (xp, yp, xf, "next");
%% pch = interp1Custom (xp, yp, xf, "pchip");
%% spl = interp1Custom (xp, yp, xf, "spline");
%% plot   (xf,yf,"r" , xf,near,"g", xf,lin,"b", xf,previous,"c", xf,next,"m", xp,yp,"r*");
%% legend ("original", "nearest"  , "linear"  , "previous"     , "next"     , "sampled");
%% @end group
%% @end example
%%
...
function yi = interp1Custom (x, y, varargin)

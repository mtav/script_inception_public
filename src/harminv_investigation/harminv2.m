function [f,decay,Q,amp,phase,err] = harminv2( datafile, timestep, start_freq, stop_freq )
  % based on openEMS harminv wrapper, but without negative frequency hack
  % it also uses an existing input file
  %
  % [f,decay,Q,amp,phase,err]=harminv( datafile, timestep, start_freq, stop_freq )
  %
  ...
  command = [harminv_exe ' ' options ' '...
             num2str(start_freq) '-' num2str(stop_freq) ' < "' datafile '"'];

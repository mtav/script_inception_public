function h = mArrow3(p1, p2, varargin)
  ...
  %                       'stemWidth':  width of the line (actually radius of the cylinder), default: stemWidth = 1/300*(diagonal of axis box)
  %                       'tipWidth':   width of the cone (actually radius of the cone), default: tipWidth = 3*stemWidth
  %                       'tipLength': length (height) of the cone, default: tipLength = tipWidth/tan(tipAngle/2), where tipAngle = 22.5 degrees is the double angle of the cone tip
  %
  ...
  tipAngle = 22.5/180*pi; % 22.5 degrees in radians
  ...
  %% only cone v
      ...
      tipWidth = 2*sin(tipAngle/2)*norm(p2-p1); % seems a little bit arbitrary, but it basically leads to a wider cone...
      % vertices of the tip circle
      for idx = 1:ppbc+1

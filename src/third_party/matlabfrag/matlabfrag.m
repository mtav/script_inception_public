...
function matlabfrag(FileName,varargin)
  ...
  p = inputParserWrapper(p, 'addRequired', 'FileName', @(x) ischar(x) );
  p = inputParserWrapper(p, 'addParamValue', 'handle', gcf, @(x) ishandle(x) && strcmpi(get(x,'Type'),'figure') );
  p = inputParserWrapper(p, 'addParamValue', 'epspad', [0,0,0,0], @(x) isnumeric(x) && (all(size(x) == [1 4])) );
  p = inputParserWrapper(p, 'addParamValue', 'renderer', 'painters', @(x) any( strcmpi(x,{'painters','opengl','zbuffer'}) ) );
  p = inputParserWrapper(p, 'addParamValue', 'dpi', 300, @(x) isnumeric(x) );
  p = inputParserWrapper(p, 'addParamValue', 'debuglvl',0, @(x) isnumeric(x) && x>=0);

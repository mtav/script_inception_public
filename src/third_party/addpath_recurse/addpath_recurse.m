function addpath_recurse(strStartDir, caStrsIgnoreDirs, strXorIntAddpathMode, blnRemDirs, blnDebug)
  ...
  % Note:
  % =====
  % "addpath(genpath(p))" and "rmpath(genpath(p))" are a MATLAB/Octave built-in way to add/remove paths recursively. However, they do not offer a way to exclude directories.
  % "genpath" from GNU Octave actually does allow excluding certain directories and is a built-in function from the file libinterp/corefcn/load-path.cc
  % This script currently does not work properly on Matlab 2014 for a still unknown reason.
  % TODO: Fix for Matlab 2014 + maybe rewrite in a simpler way, by simply filtering genpath output.
  % TODO: Create Octave-like genpath with option to ignore certain directories. This allows usage for other purposes than just adding/removing paths.
  %
  % See Also
  % ========
  % addpath()

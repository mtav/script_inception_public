function tah = addtxaxis(ah,transform,ticks,txlabel,ticklabels);
  ...
  %
  % Examples:
  % ah = plot(rand(1,10)); tah = addtxaxis(gca,'get_c0()./x',get_c0()*[1/10,1/9,1/5,1/2]);
  % t=-10:10; ah = plot(t, t.^2); xlabel('t'); tah = addtxaxis(gca, '1./x', 1./t, '1/t');
  %
  % TODO: Get tickmarks automatically
  % TODO: Positioning options for new axis (and enabling use of more than 1 axis if desired, but maybe that's too much...)
...
function tah = set_transformed_axes(ah,transform,ticks,txlabel,ticklabels);
  ...
  % link the axes, so that that both changes when panning/zooming (a callback would be necessary to also update the ticks, but first the ticks should be acquired from the original axis of course)
  linkaxes([ah, tah],'xy');

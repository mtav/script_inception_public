...
%usual magnification values are 15k, 20k, 25k, 35k, 50k, 65k, 80k and 100k

dw=400000 %dwell in 10ths of a microsecond
r=[];d=[];

fstr = fopen('test_annulus.str','wt');
...
xc=round(xc./pixel)+1000; %put pattern near centre of screen
...
for rad=1:numel(r2)./2
    ...
    radius2=pixelx^2
    radius3=round(radius2*0.5)%inner radius values
    for y=-pixelx:pixelx;
        for x=-pixelx:pixelx;
            if (radius3<=(x^2+y^2)) && ((x^2+y^2)<radius2), c(a,1)=x;c(a,2)=y;
               a=a+1;       
            end
        end
    end

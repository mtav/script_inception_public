...
def plotBandsAll(symmetric_fill=False, highlight_bandgap=False, outfile=None):
  plt.figure(figsize=(10,8))
  ...
  plt.legend([lines_odd[0], lines_even[0]], ['TM odd bands', 'TM even bands'])
  if outfile:
    outfile_final=outfile+'.png'
    print(f'Saving as {outfile_final}')
    plt.savefig(outfile_final)
...
#plotBandsAll(symmetric_fill=False)
plotBandsSingle(kx2, tmyeven_freqs2, tmyodd_freqs2, outfile=outfile_base+'.bands')
...
(ms2, kx2, tmyeven_freqs2, tmyodd_freqs2) = computeBands(wx_n, wy_n)
showGeom(ms2, outfile=outfile_base)
plotBandsAll(symmetric_fill=True, highlight_bandgap=True, outfile=outfile_base+'.bands')

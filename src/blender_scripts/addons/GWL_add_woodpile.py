...
bl_info = {
    "name": "New GWL woodpile (not yet working!!!) - EXPERIMENTAL",
    "author": "mtav",
    "version": (1, 0),
    "blender": (2, 65, 0),
    "location": "View3D > Add > Mesh > Add GWL Woodpile",
...
class OBJECT_OT_add_woodpile(Operator, AddObjectHelper):
...
def add_object_button(self, context):
    self.layout.operator(
        OBJECT_OT_add_woodpile.bl_idname,
...
def register():
    bpy.utils.register_class(OBJECT_OT_add_woodpile)
    bpy.utils.register_manual_map(add_object_manual_map)
    bpy.types.INFO_MT_mesh_add.append(add_object_button)
    # bpy.types.INFO_MT_mesh_add.append(menu_func)

def unregister():
    bpy.utils.unregister_class(OBJECT_OT_add_woodpile)
